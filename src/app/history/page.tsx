...
// Same-origin watermark — same logic as app/page.tsx
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm text-white/50">WheelVision AI Sonucu</span>
          <div className="flex items-center gap-2">
            {!isPaid && (
              <Link
                href="/pricing"
                onClick={(e) => e.stopPropagation()}
                className="flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium text-[var(--accent-orange)] border border-[var(--accent-orange)]/40 hover:bg-[var(--accent-orange)]/10 transition-colors"
              >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold text-white transition-all hover:scale-105 hover:shadow-[0_0_24px_rgba(247,37,133,0.5)]"
              style={{ background: 'linear-gradient(135deg, #FF6B35, #F72585)' }}
            >
              <Download className="w-4 h-4" />
              İndir {!isPaid && '(Filigranla)'}
            </button>
            <button
              onClick={onClose}
        ...
        <div className="relative">
          <img
            src={url}
            alt="AI Sonucu"
            className="max-h-[78vh] w-auto mx-auto object-contain rounded-2xl shadow-2xl"
          />
          {/* Free plan watermark overlay hint */}
          {!isPaid && (
            <div className="absolute inset-0 flex items-end justify-center pb-4 pointer-events-none">
              <div className="px-3 py-1.5 rounded-full bg-black/60 backdrop-blur-sm text-xs text-white/70">
                Filigransız indirmek için <span className="text-[var(--accent-orange)]">ücretli plana geç</span>
              </div>
            </div>
          )}
        </div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [genResult, userResult] = await Promise.all([
        supabase
          .from('generations')
          .select('id, user_email, result_image_url, created_at')
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="mb-8 flex items-end justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold mb-1">
                Görselleştirme <span className="gradient-text">Geçmişi</span>
              </h1>
              <p className="text-[var(--text-secondary)] text-sm">
                {generations.length > 0 ? `${generations.length} sonuç` : ''}
              </p>
            </div>
            ...
            {!paid && generations.length > 0 && (
              <Link
                href="/pricing"
                className="flex items-center gap-2 px-4 py-2.5 rounded-full border border-[var(--accent-orange)]/40 bg-[var(--accent-orange)]/5 text-sm hover:bg-[var(--accent-orange)]/10 transition-colors"
              >
                <Lock className="w-4 h-4 text-[var(--accent-orange)]" />
                <span>
                  <span className="text-[var(--accent-orange)] font-medium">Ücretli plana geç</span>
                  <span className="text-[var(--text-secondary)]"> → tüm filigranlar kalkar</span>
                </span>
              </Link>
            )}
          </div>
          ...
          {generations.length === 0 && (
            ...
            >
              ...
              <h2 className="text-xl font-semibold mb-2">Henüz bir görselleştirme yapmadınız</h2>
              <p className="text-[var(--text-secondary)] mb-8 max-w-sm">
                İlk jant görselleştirmenizi oluşturmak için uygulamaya gidin.
              </p>
              <button
                onClick={() => router.push('/app')}
                className="btn-primary flex items-center gap-2"
              >
                Görselleştirmeye Başla
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {generations.map((gen, index) => (
                <motion.div
                  ...
                  onClick={() => gen.result_image_url && setModalUrl(gen.result_image_url)}
                >
                  {/* Square image */}
                  ...
                  {/* Date */}
                  <p className="mt-1.5 text-[11px] text-[var(--text-secondary)] text-center opacity-60 truncate px-1">
